...
# ✅ Discuss agent
class DiscussAgent:
    def __init__(self, system_message: SystemMessage, model: ChatGoogleGenerativeAI):
...
task_specify_agent = DiscussAgent(
    ...
    ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=1.0),
)


# ✅ Main function to generate syllabus
def generate_syllabus(topic, task):
    ...
    assistant_agent = DiscussAgent(
        assistant_sys_msg, ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0.2)
    )
    user_agent = DiscussAgent(
        user_sys_msg, ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0.2)
    )
    ...
    summarizer_prompt = """Here is a conversation history that {assistant_role_name} had with {user_role_name}: {conversation_history}.
    Please summarize this conversation into a course syllabus with the given topic."""
